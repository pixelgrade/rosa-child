<?php
/**
 * Rosa Child functions and definitions
 *
...
/**
 * Setup Rosa Child Theme's textdomain.
 *
 * Declare textdomain for this child theme.
 * Translations can be filed in the /languages/ directory.
 */
function rosa_child_theme_setup() {
	load_child_theme_textdomain( 'rosa-child-theme', get_stylesheet_directory() . '/languages' );
}
add_action( 'after_setup_theme', 'rosa_child_theme_setup' );
...
function rosa_child_enqueue_styles() {
	...
	if ( is_rtl() ) {
		wp_enqueue_style( 'rosa-rtl-style', get_template_directory_uri() . '/rtl.css', array(), $parent->get( 'Version' ) );
	} else {
		wp_enqueue_style( 'rosa-main-style', get_template_directory_uri() . '/style.css', array(), $parent->get( 'Version' ) );
	}
...
function rosa_child_overwrite_files() {
	...
	// See the parent's function.php -> the rosa_scripts_styles() function
	// for details like resources names

		wp_dequeue_script( 'rosa-main-scripts' );
	...
	// This is not required, but I assume you are not modifying that much :)
		...
		wp_enqueue_script( 'rosa-child-scripts',
	...
	// see the parent's function.php -> the hive_scripts_styles() function for details like resources names
		...
		wp_dequeue_style( 'rosa-main-style' );
...
// Load the files from the function mentioned above:
	...
	add_action( 'wp_enqueue_scripts', 'rosa_child_overwrite_files', 11 );
